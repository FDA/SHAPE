...
admin.initializeApp(functions.config().firebase, 'message');
...
export class MessageService {
    ...
    private processPromises (values: any[], callback: CallbackFunction) {
        ...
        let initialMerge = [...values[0], ...values[1].filter((elem: any) => !ids.has(elem.id))];
        ids = new Set(initialMerge.map((elem: any) => elem.id));
        let finalMerge = [...initialMerge, ...values[2].filter((elem: any) => !ids.has(elem.id))];
    ...
    public query(org: string, participantId: any, callback: CallbackFunction) {
        if(org === "ALL") {
            ...
            const inAppPromise = this.createALLPromise("pushRecipients", participantId, callback);
            ...
            const inAppPromise = this.createORGPromise("pushRecipients", participantId, org, callback);
