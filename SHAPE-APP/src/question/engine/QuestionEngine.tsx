...
export class QuestionEngine {
  ...
  public getQuestionTitle(name: string): string {
    let question = this._questions.filter((q) => q.name === name);
  ...
  public isValid(currentValue: string): boolean | undefined {
    ...
    return validationFactory.validate(q, currentValue, this._context);
  }

  public getCurrentMessage(): string {
    const q = this._questions[this._currentQuestion];
    let retVal = "This question is required.";

    const { requiredMessage } = q;
    if (!requiredMessage) {
      return retVal;
    }
