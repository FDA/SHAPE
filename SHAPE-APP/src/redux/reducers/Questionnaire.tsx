...
let questionnaireData: Array<any> = [];
let questionnairesState: Array<any> = [];

let initialState = questionnaireData
  ? questionnaireData
  : { questionStack: [] };
let initialResponseState: Array<any> = [];
...
export function applicationReady(
  state: boolean = false,
  action: { type: string; isReady: boolean | undefined }
) {
  switch (action.type) {
...
export function surveys(
  state: any = [],
  action: { type: string; data: Array<Survey> }
) {
  switch (action.type) {
    case SURVEY_QUERY:
      const { data } = action;
...
export function selectedProfile(
  state: any = {},
  action: { type: string; data: Person }
) {
  switch (action.type) {
...
export function participantResponse(
  ...
  switch (action.type) {
    ...
    case Q13_ANSWER_WRITE:
      ...
      const previousAnswerIndex = newState.findIndex(
