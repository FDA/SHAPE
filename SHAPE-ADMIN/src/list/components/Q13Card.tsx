...
import { getQuestionnaireResponseJSONExport, getQuestionnaireResponseFHIRExport } from '../../utils/API';
...
export const Q13Card: React.FC<Q13> = (q13) => {
    ...
    const downloadFile = async () => {
        getQuestionnaireResponseFHIRExport(questionnaire.id).then(async (response: any) => {
            processDownload(response);
        });
    };

    const downloadJSONFile = async () => {
        getQuestionnaireResponseJSONExport(questionnaire.id).then(async (response: any) => {
            processDownload(response);
        });
    };
    ...
    return (
        <IonCard
            ...
            <IonCardHeader style={{ backgroundColor: getColor(questionnaire.open, questionnaire.locked) }}>
                <IonCardTitle>{q13.questionnaire.name}</IonCardTitle>
                <IonCardSubtitle>{q13.questionnaire.shortDescription}</IonCardSubtitle>
            </IonCardHeader>
            <IonCardContent style={{ padding: '16px' }}>
                <IonLabel color='tertiary'>Status:</IonLabel>{' '}
                <IonText color={getTextColor(questionnaire.open, questionnaire.locked)}>
                ...
                <IonText>
                    {!isEmptyObject(q13.questionnaire.dateCreated)
                        ? format(new Date(q13.questionnaire.dateCreated), dateFormats.MMddyyZYYHHmmss)
                        : 'N/A'}
                </IonText>
                <br />
                <br />
