...
export interface State {
    ...
    min: number; //Range and Slider only
    max: number; //Range and Slider only
...
class BaseQuestion extends React.Component<ReduxProps, State> {
    ...
    deleteChoice = (index: number) => {
        ...
        tempArray.map((choice: QuestionChoice, index: number) => {
            choice.order = index;
            return choice;
        });
        ...
        switch (type) {
            ...
            case questionTypes.RADIOGROUP:
                let cList = choices.filter((choice: QuestionChoice) => {
                    return (
                        isEmptyObject(choice.value) ||
                        isEmptyObject(choice.text)
                    );
                });
